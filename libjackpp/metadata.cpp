...
void jack_free_description(jack_description_t* desc, int free_actual_description_too)
{
    uint32_t n;
...
static int jack_property_change_notify (jack_client_t* client, jack_uuid_t uuid, const char* key, jack_property_change_t change)
{
    jack_request_t req;
...
static void make_key_dbt (DBT* dbt, jack_uuid_t subject, const char* key)
{
...
int jack_get_description (jack_uuid_t         subject,
			  jack_description_t* desc)
{
    return 0;
}

int jack_get_all_descriptions (jack_description_t** descs)
{
    return 0;
}
