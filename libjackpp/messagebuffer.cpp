...
static std::atomic_uint mb_overruns { 0 };
...
jack_messagebuffer_init ()
{
    ...
    mb_overruns.store( 0, std::memory_order_release );
    mb_initialized = 1;
...
jack_messagebuffer_exit ()
{
    ...
    uint32_t mo = mb_overruns.load( std::memory_order_consume );

    if( mo > 0 ) {
	jack_error("WARNING: %ld message buffer overruns!", mo);
    }
