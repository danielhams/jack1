 ...
 *
 *   precondition: caller holds the graph lock. */
static inline void jack_sync_poll_deactivate( jack_engine_t * engine, jack_client_internal_t *client)
{
    if (client->control->sync_poll) {
...
static inline int jack_sync_timeout( jack_engine_t * engine )
{
...
int jack_timebase_set( jack_engine_t * engine, jack_uuid_t client_id, int conditional)
{
    int ret = 0;
    struct _jack_client_internal *client;
 ...
 *
 *   precondition: caller holds the graph lock. */
void jack_transport_activate( jack_engine_t * engine, jack_client_internal_t *client )
{
 ...
 *
 * precondition: caller holds the graph lock */
void jack_transport_client_exit( jack_engine_t * engine, jack_client_internal_t *client)
{
    if (client == engine->timebase_client) {
...
int jack_transport_client_reset_sync( jack_engine_t * engine, jack_uuid_t client_id)
{
    int ret;
...
int jack_transport_client_set_sync( jack_engine_t * engine, jack_uuid_t client_id)
{
    int ret;
    jack_client_internal_t *client;
...
int jack_transport_set_sync_timeout( jack_engine_t * engine, jack_time_t usecs)
{
...
int jack_timebase_reset_pp( jack::engine * engine, jack_uuid_t client_id )
{
    ...
    client = engine->client_internal_by_id( client_id );
    if( client && (client == engine->timebase_client) ) {
	client->control->is_timebase = 0;
	client->control->timebase_new = 0;
	engine->timebase_client = nullptr;
	ectl->pending_time.valid = (jack_position_bits_t)0;
	VERBOSE( engine, "%s resigned as timebase master",
		 client->control->name );
	ret = 0;
...
void jack_transport_cycle_end_pp( jack::engine * engine )
{
    ...
    /* Update timebase, if needed. */
    if( ectl->transport_state == JackTransportRolling ) {
	ectl->pending_time.frame = ectl->current_time.frame + ectl->buffer_size;
    } 
