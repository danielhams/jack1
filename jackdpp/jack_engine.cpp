...
int engine::run_cycle_pp( jack::engine * engine_ptr, jack_nframes_t nframes, float delayed_usecs )
{
	...
	*/
	...
	if( engine_ptr->run_one_cycle_( b_size, delayed_usecs ) ) {
	    jack_error( "cycle execution failure, exiting" );
	    return EIO;
	}
    }
...
int engine::run_one_cycle_( jack_nframes_t nframes, float delayed_usecs )
{
    int ret = -1;
    static int consecutive_excessive_delays = 0;
...
void engine::delay_( float delayed_usecs )
{
    jack_event_t event;
	
    control->frame_timer.reset_pending = 1;

    control->xrun_delayed_usecs = delayed_usecs;
